...
Далее под $\mathfrak{R}([0,T]; V,H)$ будем понимать множество функций $\Psi:[0,T]\to\mathcal{L}(H,V)$, таких, что
...
2) Утверждение о том, что если функция $\Psi:\Gamma\to\mathcal{L}(H,V)$ обладает свойствами 1)--4), то оператор $A$,соотношениями~\eqref{L.H->C([0,T],En2([0,T];V,H))!representations}, является линейным ограниченным оператором, действующим из $H$ в $C([0,T],\textrm{Є}([0,T];V,H))$, вытекает непосредственно из этих свойств.
\end{Proof}
...
\begin{enumerate}
    ...
    \item функция $\Psi:\Gamma\to\mathcal{L}(V,H)$ такова, что формула
    \begin{gather*}
...
%Рассмотрим теперь вопрос о представлении решения задачи Коши (\ref{abstrChauchyprobeq:nonaut}), (\ref{abstrChauchyprobinitcond:nonaut}).
%
...
%\end{gather*}
%Введём операторы $\Lambda_1:V\times H\times W^1_1([0,T], Y^*)\to \textrm{Є}([0,T];V,H)$ и $\Lambda_2:L_1([0,T],H)\to \textrm{Є}([0,T];V,H)$ по формулам
%\begin{gather*}
%\Lambda_1[v,h,g]\equiv\Lambda[v,h,0,g],\,\,\,\forall\,(v,h,g)\in V\times H\times W^1_1([0,T], Y^*);\,\,\,
%\Lambda_2[\mathfrak{f}]\equiv\Lambda[0,0,\mathfrak{f}, 0]\,\,\,\forall\,\mathfrak{f}\in L_1([0,T],H).
%\end{gather*}
%Тогда решение задачи Коши (\ref{abstrChauchyprobeq:nonaut}), (\ref{abstrChauchyprobinitcond:nonaut}) можно записать в виде
%\begin{gather*}
%\mathfrak{z}(t)=\Lambda_1[\varphi,\psi,\mathfrak{g}](t)+\Lambda_2[f](t),\,\,\,t\in[0,T].
%\end{gather*}
...
В данном разделе мы получим представление решения задачи Коши (\ref{abstrChauchyprobeq:nonaut}), (\ref{abstrChauchyprobinitcond:nonaut}). А именно, покажем, что справедлива	    
