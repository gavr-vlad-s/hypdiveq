...
Дадим следующее
\begin{Definition}
Функцию $z$ назовём решением задачи (\ref{semilinear:problem:Dirichlet2[n=1]}), принадлежащим классу $\textrm{Є}^{1}_{2,0}(Q_T)$, если $z$ является элементом $\textrm{Є}^{1}_{2,0}(Q_T)$ и удовлетворяет ин\-тег\-раль\-но\-му тождеству
